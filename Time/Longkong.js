...
!(async () => {
  if (typeof $request !== 'undefined') {
    ...
    if (/^mutation[\s\S]+__typename[\s\S]+punch[\s\S]*\{/.test(body?.query)) {
      $.setjson(
        {
          url: $request.url,
    ...
    if (!data) {
      throw new Error(
        `签到失败: ${
          $.lodash_get(body, 'errors')
            .map(i => i.message)
            .join(' ') || '未知错误'
        }`
      )
    }
    await notify(TITLE, `签到成功`, `已连续签到: ${data?.punch?.punchallday || '-'} 天`)
  }
})()
  .catch(async e => {
    $.logErr(e)
